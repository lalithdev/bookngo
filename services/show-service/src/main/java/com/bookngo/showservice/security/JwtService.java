...
public class JwtService {
    ...
    public String generateTokenWithTheatres(UUID userId, List<String> roles, List<String> assignedTheatres) {
        long nowMillis = System.currentTimeMillis();
        Date now = new Date(nowMillis);
        Date expiryDate = new Date(nowMillis + (expirationSeconds * 1000));
        ...
        return Jwts.builder()
                .subject(userId.toString())
                .claim("roles", roles)
                .claim("assignedTheatres", assignedTheatres)
                .issuedAt(now)
                .expiration(expiryDate)
                .signWith(signingKey)
                .compact();
    ...
    @SuppressWarnings("unchecked")
    public List<String> extractAssignedTheatres(String token) {
        return extractAllClaims(token).get("assignedTheatres", List.class);
    }
